...
void init_context_pss_nr(NR_DL_FRAME_PARMS *frame_parms_ue)
{
  ...
  int sizePss = LENGTH_PSS_NR * IQ_SIZE;  /* complex value i & q signed 16 bits */  /// PSS127个符号
  int size = ofdm_symbol_size * IQ_SIZE; /* i and q samples signed 16 bits */  ///时域上2048个点
  int16_t *p = NULL;
  ...
  for (int i = 0; i < NUMBER_PSS_SEQUENCE; i++) {
    ...
    if (p != NULL) {
      primary_synchro_nr[i] = p;//频域上127个符号
    ...
    p = malloc16(size);
    if (p != NULL) {
      primary_synchro_time_nr[i] = p;//时域上2048个点
    ...
    // ///写文件：输出pss时域波形
    // c16_t *out = (c16_t *)primary_synchro_time_nr[0];
    // FILE *fp1 = fopen(addr_pss, "w+");

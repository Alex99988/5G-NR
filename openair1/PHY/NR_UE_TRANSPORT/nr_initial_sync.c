  ...
  */
  ...
  if (1){ // (cnt>100)
   ...
   for(is=0; is<n_frames;is++) {
    ...
    ue->ssb_offset = sync_pos - fp->nb_prefix_samples; ///PSS位置减去循环前缀长度
    ...
    if (ue->ssb_offset + NR_N_SYMBOLS_SSB * (fp->ofdm_symbol_size + fp->nb_prefix_samples) < fp->samples_per_frame) {
      ...
      if (ret == 0) {
        ...
        sync_pos_frame = n_symb_prefix0*(fp->ofdm_symbol_size + fp->nb_prefix_samples0)+(ue->symbol_offset-n_symb_prefix0)*(fp->ofdm_symbol_size + fp->nb_prefix_samples);///symbol_offset是ssb所在符号偏移
        ...
        if (ue->ssb_offset < sync_pos_frame){
          ...
          ue->rx_offset = ue->ssb_offset - sync_pos_frame;///ssb所在帧帧开始位置
      }   

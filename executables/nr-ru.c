...
char address[60] = {"/home/zk/5G-communication/data/gNB/"};
char addr_rxdata[60], addr_txdata[60];
...
void rx_rf(RU_t *ru,int *frame,int *slot) {
  ...
  proc->tti_rx = fp->get_slot_from_timestamp(proc->timestamp_rx,fp); /////根据timestamp计算当前时隙号
...
void tx_rf(RU_t *ru,int frame,int slot, uint64_t timestamp) {
  ...
  if (flags) {
    ...
    ///写文件
    c16_t *out = (c16_t *)txp[0];
    FILE *fp1 = fopen(addr_txdata, "ab+");
    for (int i = 0; i < siglen + sf_extension; i++) {
      // if(out[i].r != 0){
...
void init_file(){
  ...
  fclose(fp1);
  strcpy(addr_txdata, address);
  strcat(addr_txdata, "txdata.iq");
  fp1 = fopen(addr_txdata, "w+");
  fclose(fp1);
}
...
void *ru_thread( void *param ) {
  ...
  while (!oai_exit) {
    ...
    if (ru->fh_south_in) ru->fh_south_in(ru,&frame,&slot);///读取一个时隙的数据
    else AssertFatal(1==0, "No fronthaul interface at south port");

    //写文件
    c16_t *out = (c16_t *)&ru->common.rxdata[0][fp->get_samples_slot_timestamp(slot, fp, 0)];
    FILE *fp1 = fopen(addr_rxdata, "ab+");
    for (int i = 0; i < ru->nr_frame_parms->samples_per_slot0; i++) {
      // if(out[i].r != 0){
      fwrite(&out[i].r, sizeof(out[i].r), 1, fp1);
      fwrite(&out[i].i, sizeof(out[i].i), 1, fp1);
      //}
    ...
    int sl=proc->tti_tx;
    for (int slidx=0;slidx<ru->sl_ahead;slidx++) ///定时提前

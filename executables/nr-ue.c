...
#include "executables/my_tool.h"
char address[60] = {"/home/zk/5G-communication/data/UE/"};
char addr_rxdata[60], addr_rxdataF[60], addr_rxdataRB[60], addr_pss[60];
      ...
      for (int i=0; i<UE->frame_parms.nb_antennas_rx; i++) {
        if (toTrash)
          ...
          rxp[i] = ((void *)&UE->common_vars.rxdata[i][0]) +
                   4*((x*UE->frame_parms.samples_per_subframe)+//一个样点4个字节所以*4
...
void init_file(){
  strcpy(addr_rxdata, address);
  strcat(addr_rxdata, "rxdata.iq");
  FILE* fp1 = fopen(addr_rxdata, "w+");
  fclose(fp1);
  strcpy(addr_rxdataF, address);
  strcat(addr_rxdataF, "rxdataF.iq");
  fp1 = fopen(addr_rxdataF, "w+");
  fclose(fp1);
  strcpy(addr_rxdataRB, address);
  strcat(addr_rxdataRB, "rxdataRB.bin");
  fp1 = fopen(addr_rxdataRB, "w+");
  fclose(fp1);
  strcpy(addr_pss, address);
  strcat(addr_pss, "pss.iq");
  fp1 = fopen(addr_pss, "w+");
  fclose(fp1);
}
...
void *UE_thread(void *arg) {
  ...
  notifiedFIFO_t nf;///任务结束后结果返回队列地址  
  initNotifiedFIFO(&nf);

  notifiedFIFO_t freeBlocks;///待入线程池的任务队列
  ...
  while (!oai_exit) {
    ...
    if (syncRunning) {
      notifiedFIFO_elt_t *res=tryPullTpool(&nf,&(get_nrUE_params()->Tpool)); /// 获得UE_synch结果
    ...
    if (!UE->is_synchronized) {
      ...
      pushTpool(&(get_nrUE_params()->Tpool), Msg);/// 同步UE_synch进入线程
      trashed_frames=0;
      syncRunning=true;
      continue;
    }
    ...
    if (start_rx_stream == 0) {
      ...
      ///写文件
      c16_t *out = (c16_t *)UE->common_vars.rxdata[0];
      FILE *fp1 = fopen(addr_rxdata, "ab+");
      for (int i = 0; i < UE->frame_parms.ofdm_symbol_size + UE->frame_parms.nb_prefix_samples0; i++) {
        // if(out[i].r != 0){
      ...
      fclose(fp1);
      slot_to_freq(UE, UE->common_vars.rxdata[0], 0, 1, (absolute_slot + 1) % nb_slot_frame);
    ...
    for (int i = 1; i < UE->frame_parms.symbols_per_slot; i++) {
      slot_to_freq(UE, rxp[0], i, 0, slot_nr);
    }
    if (slot_nr != (UE->frame_parms.slots_per_frame - 1)) {
      slot_to_freq(UE, rxp[0], 0, 0, slot_nr);
    }
...
void init_NR_UE_threads(int nb_inst) {
  ...
  for (inst=0; inst < nb_inst; inst++) {
    PHY_VARS_NR_UE *UE = PHY_vars_UE_g[inst][0];  //UE物理层参数

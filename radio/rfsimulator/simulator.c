  ...
  for (int nbEv = 0; nbEv < nfds; ++nbEv) {
    ...
    if (events[nbEv].events & EPOLLIN && fd == t->listen_sock) {
      ...
      if ( b->headerMode)
        ...
        blockSz= b->transferPtr + b->remainToTransfer <= b->circularBufEnd ?  ///判断剩余需要传输的是否超过buf大小
                 b->remainToTransfer :
      ...
      if (b->transferPtr==b->circularBufEnd ) ///指向buf开始
        b->transferPtr=(char *)b->circularBuf;

      // check the header and start block transfer
      if ( b->headerMode==true && b->remainToTransfer==0) {///接收到header
        ...
        } else if ( b->lastReceivedTS < b->th.timestamp) {///考虑上下行情况
